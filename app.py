...
def gpt_analyze(step, user_text, history, infos, user_fields):
    try:
        ...
        3. ثالثاً: إذا كانت الرسالة مناسبة للمرحلة الحالية، جاوب في السطر الأول فقط بـ: "نعم"
        ⚠️ ضروري تكون كلمة "نعم" هي أول كلمة في الإجابة إذا كانت مناسبة للمرحلة.
        4. بعدها يمكنك إضافة شرحك وتحليلك الكامل إذا لزم.
        5. إذا كانت الرسالة استفسار عن شيء في المتجر (السعر، التوصيل، الدفع، المقاسات...) جاوب عليه بدارجة جزائرية مفهومة.
...
def webhook():
    ...
    if not client:
        ...
        response_intro = f""مرحبا بيك خويا
أنا هنا نعاونك باش نكملو الطلب تاعك على راحتك.
📌 باش نقدر نعاونك، نبداو بجمع بعض المعلومات البسيطة:
- أول حاجة نحتاجو رمز المنتج لي عجبك (مثلاً G09).
- ومن بعد نكملو برقم الهاتف و العنوان.
