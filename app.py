...
def gpt_analyze(step, user_text, history, infos, user_fields):
    try:
        ...
        5. إذا كانت الرسالة استفسار عن شيء في المتجر، جاوب عليه بدارجة جزائرية مفهومة.
        6. إذا كانت غير مفهومة أو غير مرتبطة، جاوب بـ: "ما فهمتش، واش تقصد؟"
        7. لا تعيد طرح الأسئلة. لا تفترض معلومات غير موجودة. لا تخرج عن السياق.
        """
...
def webhook():
    ...
    if not user_text:
        send_message(sender_id, "بعتلنا الكتابة فقط من فضلك.")
        return "ok"
...
- أول حاجة نحتاجو رمز المنتج لي عجبك (مثلاً G09).
