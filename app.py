...
def handle_user_message(user_id, user_text, previous_messages, summary, product_list):
    ...
    system_prompt = f"""
أنت مساعد افتراضي (شات بوت) لمتجر أقمصة النور في الجزائر.
- تتكلم باللهجة الجزائرية شبه الرسمية فقط بلا إيموجي.
- هذه قائمة المنتجات المتاحة فقط (لا تخترع أي منتج آخر):
{products_json}

- إذا لم تفهم كلام العميل، استخدم:
\"intent\": \"need_clarification\",
\"message_for_user\": \"ما فهمتش قصدك بالضبط انا غي مساعد آلي راني هنا باش نعاونك\"

-  إذا أراد شراء منتج، أطلب منه تأكيد معطرح الاسئلة اللازمة لتأكيد الطلب وتسجيله في الجدول (confirm_purchase = true) قبل تسجيل الطلب.
